...
struct func_stroff_t : public action_handler_t
{
  virtual int idaapi activate(action_activation_ctx_t *ctx)
  {
    ...
    struct set_union_sel_t : public ui_stroff_applicator_t
    {
      ...
      virtual bool idaapi apply(size_t opnum, const intvec_t &path) override
      { // save the user selection
        vu.cfunc->set_user_union_selection(eas[opnum], path);
        vu.cfunc->save_user_unions();
        return true;
      }
    };
    set_union_sel_t su(vu, eas);
...
static const char ah_name[] = "vds17:strchoose";
static const action_desc_t stroff_desc = ACTION_DESC_LITERAL(
        ah_name,
        "Structure offsets",
        &func_stroff_ah,
        "Shift-T",
