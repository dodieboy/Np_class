...
//        INTERR(50769);
        ...
        if ( !mv.mba->lvar_alloc_failed() )
        {
            ...
            && !v.is_mapdst_var() ) // relax the check for mapdsts because map destination may eventually shrink
          {
            ...
            { // allow address references past end of item: &buf[sizeof(buf)]
              if ( v.width < l->off )
                INTERR(50777); // reference past end of variable
            }
            else
            { // allow references only in the middle of the item
              if ( v.width <= l->off )
                INTERR(50778); // reference past end of variable
            }
          }
          if ( l->off < 0 )
            INTERR(50779); // reference before start of variable
        }
      }
      break;
    case mop_a: // address of variable (mop_l, mop_v, mop_S, mop_r)
      ...
      {
        // addresses of registers are allowed only in helper functions
    ...
    case mop_p:
      ...
      if ( mv.blk != NULL )
      {
        ...
        QASSERT(52045, mv.mba->lvars_allocated()
...
void minsn_t::verify(micro_verifier_t &mv, bool with_target) const
{
  ...
  switch ( opcode )
  {
    ...
    case m_sar:
      if ( r.size != 1 )
        INTERR(50835);
      if ( r.t == mop_n
        && mvm.get_shift_mask != NULL
        && uint8(r.nnn->value) > mvm.get_shift_mask(l.size) )
      {
        INTERR(52118);
      }
      // no break
  ...
  if ( opcode != m_ijmp && opcode != m_stx && opcode != m_ext )
  {
    switch ( d.t )
    {
      case mop_d: // d cannot be another insn
...
void mbl_array_t::verify_args(void) const
{
  ...
  for ( int i=0; i < argidx.size(); i++ )
  {
    ...
    if ( is_user_cc(cc) && nargs() < hrcfg.max_func_args )
    {
...
//-------------------------------------------------------------------------
void mbl_array_t::verify_lvars(bool check_args) const
{
  strings_t names;
  ...
  for ( int i=0; i < vars.size(); i++ )
  {
    ...
    if ( retvaridx == i )
    {
      if ( v.location.has_stkoff() )
        INTERR(50899); // cannot have any stack part
      if ( !v.is_result_var() )
        INTERR(50900);
    }
    else
